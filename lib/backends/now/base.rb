  ...
  module Now
    class Base
      ...
      def read_fixtures(base_path)
        @logger.debug "[Backends] [Now] Reading fixtures from #{base_path.to_s.inspect}"
...
def read_#{fixture}_fixtures(path = '')
  #{fixture} = Rails.env.test? ? @#{fixture} : @dalli_cache.get('now_#{fixture}')

  unless #{fixture}
    path = path_for_fixture_file(path, :#{fixture})
    @logger.debug "[Backends] [Now] Reloading #{fixture} fixtures from '" + path.to_s + "'"
...
def save_#{fixture}_fixtures(#{fixture})
  Rails.env.test? ? @#{fixture} = #{fixture} : @dalli_cache.set('now_#{fixture}', #{fixture})
end
...
def drop_#{fixture}_fixtures(lite = true)
  if lite
    ...
    Rails.env.test? ? @#{fixture} = nil : @dalli_cache.delete('now_#{fixture}')
  end
end
|
      end
...
def read_#{fixture_tpl}_fixtures(path = '')
  #{fixture_tpl} = Rails.env.test? ? @#{fixture_tpl} : @dalli_cache.get('now_#{fixture_tpl}')

  unless #{fixture_tpl}
    path = path_for_fixture_file(path, :#{fixture_tpl})
    @logger.debug "[Backends] [Now] Reloading #{fixture_tpl} fixtures from '" + path.to_s + "'"
...
def save_#{fixture_tpl}_fixtures(#{fixture_tpl})
  Rails.env.test? ? @#{fixture_tpl} = #{fixture_tpl} : @dalli_cache.set('now_#{fixture_tpl}', #{fixture_tpl})
end
|
      end
