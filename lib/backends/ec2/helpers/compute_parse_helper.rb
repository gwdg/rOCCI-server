    ...
    module Helpers
      module ComputeParseHelper
        ...
        def compute_parse_backend_obj(backend_compute, reservation_id)
          ...
          os_tpl_mixin = resource_tpl_get(resource_tpl_list_itype_to_term(backend_compute[:instance_type]))
          if os_tpl_mixin
        ...
        def compute_parse_links_network(backend_compute, compute)
          ...
          if intfs.empty?
            ...
            intfs.each { |intfc| result_network_links << compute_parse_link_networkinterface(compute, intfc) }
          end
