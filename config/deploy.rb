...
if ENV['ROCCI_HTTP_SERVER'] && !ENV['ROCCI_HTTP_SERVER'].empty?
  ...
  role(:web) { Capistrano::CLI.ui.ask('Where is your HTTP server running?  ') do |q|; q.default = 'localhost'; end }
end

if ENV['ROCCI_APP_SERVER'] && !ENV['ROCCI_APP_SERVER'].empty?
  role :app, ENV['ROCCI_APP_SERVER']
else
  role(:app) { Capistrano::CLI.ui.ask('Where is your APP server running?  ') do |q|; q.default = 'localhost'; end }
end

if ENV['ROCCI_DB_SERVER'] && !ENV['ROCCI_DB_SERVER'].empty?
  role :db, ENV['ROCCI_DB_SERVER'], :primary => true
else
  role(:db, :primary => true) { Capistrano::CLI.ui.ask('Where is your DB server running?  ') do |q|; q.default = 'localhost'; end }
end
